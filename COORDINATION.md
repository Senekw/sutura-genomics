...
**Fix (layered — skip the bad pair first, watchdog as backstop, plus visibility):**
- **Per-pair alignment timeout** (`cli/sutura_cli/core/tools.py`): each pair's engine call runs under `_call_with_timeout`; if it exceeds `SUTURA_ALIGN_TIMEOUT` (default 150s) it raises `AlignError`, which the agent's existing `except (LoaderError, AlignError)` **skips-and-continues** (one bad pair never sinks the job; reconstruction builds from the contiguous good chain). The wedged solve keeps running in a leaked daemon thread (Python can't safely kill it) but no longer blocks the pipeline. This also hardens headless/TUI, not just `--live`.
- **Terminal now shows progress.** `serve_live` tees the SAME event stream to the CLI's `ConsoleSink` — `--live` renders load→QC→Alignment table→3D→report→"Run complete" card, identical to headless, plus a final `✓ alignment complete` / `✗ did not finish (…)` line.
- **Watchdog + heartbeat + backstop timeout** (`live.run_live` rewritten): pipeline runs in a worker thread; the main thread supervises via a `WatchedSink` that stamps last-event time + current stage. If no event for `SUTURA_LIVE_STAGE_TIMEOUT` (default **240s**, kept above the 150s per-pair limit so the skip fires first) or the run exceeds `SUTURA_LIVE_RUN_TIMEOUT` (default 5400s), it publishes an `error` (browser + terminal) + `end` and returns — **surfaces instead of hanging** (this is the backstop for a one-pair run or a hang in load/QC/reconstruct that the per-pair skip can't reach). A `heartbeat` every `SUTURA_LIVE_HEARTBEAT` (default 10s) proves liveness. `run_live` returns an outcome dict; `serve_live`'s exit code reflects it.
...
- **Watchdog unit tests** (hermetic, fake Session): a stalled stage surfaces `error("stalled")`+`end` in ~2s (no hang); a slow-but-progressing stage emits ≥2 heartbeats and completes; clean run ends `status=complete`. Overflow test proves the newest events survive. Per-pair `_call_with_timeout` tested: returns value / propagates errors verbatim / raises `AlignError` on stall in ~1s / disabled when 0.
...
- **12-section large-input, BEFORE the per-pair timeout:** reproduced the real freeze — pair 2's PASTE2 wedged; the live watchdog fired at 180s and surfaced `✗ stage 'align' stalled` on BOTH the terminal (with 27 `… still in 'align' (Ns)` heartbeats leading up to it) and the browser (`error`+`end`) instead of hanging forever.
- **12-section large-input, WITH the per-pair timeout:** <RESULT PENDING>
- Tests: `app/tests/test_live.py` +5; full app suite 19 passed; cli suite <RESULT PENDING>.
- Files: `app/sutura_app/live.py`, `server.py`, `static/live.{js,css}`, `tests/test_live.py`, **`cli/sutura_cli/core/tools.py`** (per-pair timeout). Env knobs: `SUTURA_ALIGN_TIMEOUT`, `SUTURA_LIVE_STAGE_TIMEOUT`, `SUTURA_LIVE_RUN_TIMEOUT`, `SUTURA_LIVE_HEARTBEAT`.
- **Note:** there is no literal 35-section folder in the repo; the largest homogeneous set is `data/` (12 DLPFC sections → 11 pairs), used as the large-input test.
