...
> Input: _Visium HD of human lung, disease vs control (IPF fibrosis), 5 donors per group, want cell types, fibrotic spatial domains, and differential expression between conditions_

## What we understood from your description

| Field | Value |
|---|---|
| Platform | 10x Visium HD (confidence: high) |
| Experiment type | Disease vs control comparison (confidence: high) |
| Tissue | lung |
| Serial sections | no |
| Samples/donors/conditions | 5 |
| Stated goals | cell_typing, spatial_de, spatial_domains |
...
3. **Cell segmentation** → bin2cell ⚠️
...
9. **Spatially variable genes & differential expression** → PyDESeq2 (pseudobulk DE) ⚠️
...
> ⚠️ Platform note: Bin-to-cell assignment is an active, imperfect area. 2um bins are sparse; naive per-bin clustering is noisy.
...
**Recommended: bin2cell  (python, maturity: emerging)**
  - Why: Purpose-built for Visium HD: uses the H&E/DAPI image (via StarDist) plus 2um bin expression to reconstruct cell-like objects from 2um bins. The current practical answer for 'cells' from Visium HD.
  - Install: `pip install bin2cell`
  - Run:

```python
import bin2cell as b2c
adata = b2c.read_visium('binned_outputs/square_002um/', source_image_path='he.tif')
b2c.scaled_he_image(adata)
b2c.stardist(...)  # nucleus labels
cdata = b2c.bin_to_cell(adata, ...)
```
  - Expected runtime: Minutes to hours depending on section size and StarDist backend.
  - Known limitations: Bin-to-cell is inherently approximate; boundaries come from nuclei expansion, not true membranes. Sparse 2um bins mean noisy per-cell counts. Active research area.
  - Docs: https://github.com/Teichlab/bin2cell
...
_Why here:_ You have 5 sections/samples; multiple sections/samples must be integrated into a shared embedding so that differences are biology, not batch.
...
> ⚠️ Disease-vs-control danger: integration can 'correct away' the disease effect you are testing. Integrate for clustering/annotation only; do statistics on uncorrected counts via pseudobulk (see the DE step).
...
_Why here:_ A condition comparison needs statistically defensible differential expression between conditions.
...
**Recommended: PyDESeq2 (pseudobulk DE)  (python, maturity: established)**
...
pdata = dc.get_pseudobulk(adata, sample_col='sample', groups_col='cell_type')
dds = DeseqDataSet(adata=pdata, design_factors='condition'); dds.deseq2()
stat = DeseqStats(dds, contrast=['condition','disease','control']); stat.summary()
```
  - Expected runtime: Seconds to minutes (operates on the small pseudobulk matrix).
  - Known limitations: Needs biological replicates (multiple samples per condition) - with n=1 per group there is no valid statistic. Cell-type assignment errors propagate. Not spatial per se; pair with spatial context.
  - Docs: https://github.com/owkin/PyDESeq2
...
> ⚠️ HONEST LIMIT: do NOT run per-cell/per-spot DE across conditions - it pseudo-replicates and massively inflates false positives. Pseudobulk per biological replicate (sample x cell type) and require multiple samples per condition.
