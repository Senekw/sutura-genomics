"""Overlay Sutura's cross-tear registration-error curve on the PASTE2 tear axes.
...
Left  : head-to-head median reg-err vs severity (PASTE2 tear vs Sutura).
Right : Sutura's own median / mean / p90 — shows the tear concentrates error in
...
ax1.plot(arca["severity"], arca["reg_err_median"], "o-", color="tab:blue",
         lw=2, ms=7, label="Sutura (GNN) tear")
...
ax1.set_title("Sutura vs PASTE2 on torn tissue (same warps, same GT)")
ax1.set_ylim(0, None)
ax1.grid(alpha=0.3)
ax1.legend()
...
# --- right: Sutura error distribution (where tearing bites) ---
...
ax2.set_ylabel("Sutura registration error (px)")
ax2.set_title("Sutura error distribution — tail grows with the tear")
ax2.set_ylim(0, None)
ax2.grid(alpha=0.3)
ax2.legend()
...
# console summary
print("\nseverity |  PASTE2 med |   Sutura med | Sutura mean | Sutura p90")
...
print(f"Sutura   tear median: {a0:.0f} -> {a8:.0f} px  (+{a8-a0:.0f}, +{100*(a8-a0)/a0:.0f}%)")
print(f"Sutura median lower by {p8/a8:.1f}x at sev8, {p0/a0:.1f}x at sev0.")
