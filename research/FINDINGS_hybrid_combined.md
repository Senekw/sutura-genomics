...
_Generated 2026-07-17T07:57:43Z on branch `hybrid-combined`._
...
**Benchmark.** 3-donor DLPFC leave-one-donor-out (Br5292/Br5595/Br8100), tear benchmark, median registration error in spot-pitches, eval severities 0,1,2,3,4,6,8 (tear=True, eval-seed 0) ? identical to generalization_max.py so numbers overlay prior work. PASTE2 held-out reference (full-res FGW, prior sweep): Br5292 5.28, Br5595 4.35 (mean 4.81); shared-basis plateau 9.6.


## Headline

Best configuration: **`shared_basis`** at **9.63** LODO-mean pitches, which does **not** beat PASTE2 (mean 4.81). Shared-basis baseline recomputed here for reference.


## LODO ranking (mean held-out error, lower=better)

| rank | config | LODO-mean | vs PASTE2 | Br5292 | Br5595 |
|---|---|---|---|---|---|
| 1 | `shared_basis` | 9.63 | +4.82 | 10.328 | 8.941 |
| 2 | `hybrid_no_ssa` | 11.51 | +6.70 | 11.429 | 11.598 |
| 3 | `hybrid_full` | 11.84 | +7.02 | 12.383 | 11.293 |
| 4 | `residual_ssa` | 12.02 | +7.21 | 12.592 | 11.45 |
| 5 | `ot_only` | 12.09 | +7.27 | 12.631 | 11.543 |
| 6 | `residual` | 12.38 | +7.56 | 12.292 | 12.458 |
| 7 | `piecewise_only` | 12.95 | +8.14 | 13.269 | 12.632 |

## What each component contributes

- **OT prior alone** (`ot_only`): 12.09 ? the training-free PASTE2-style correspondence surrogate (expression entropic-OT).
- **+ tear-detect/piecewise** (`piecewise_only`): 12.95 ? structural, training-free tear correction on top of OT.
- **+ learned residual, synthetic-trained** (`residual`): 12.38 ? the synthetic-tear-transfer test (trained on TRAIN donors' synthetic tears, evaluated on the real held-out donor).
- **+ self-supervised adaptation** (`residual_ssa`): 12.02.
- **full hybrid, no SSA** (`hybrid_no_ssa`): 11.51 ? OT+piecewise+residual fused by the agentic advisor gate.
- **full hybrid** (`hybrid_full`): 11.84.
- **shared-basis NN** (`shared_basis`): 9.63 (prior plateau reference 9.6).

## Synthetic-data transfer

Training the residual purely on synthetic tears of the training donors and evaluating on the real held-out donor moves error 12.09 -> 12.38 pitches. Synthetic training **does NOT transfer (no improvement over the OT base on held-out real tissue)**.
